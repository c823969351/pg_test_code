...
#################################电源协议指令##################################################
    ...
    def powerVolt(self, ch, v):
        self.send("DEVICE:POWER:VOLT %s,%f" % (ch, v))
    ...
    # read  CURRENT
    def powerQBcurrent(self, ch, v="B"):
        dats = self.query("POWE:CURR? %s,%s" % (ch, v))
        dats = float(dats.split()[0])
        return dats
        
    def powerQScurrent(self, ch, v="S"):
        dats = self.query("POWE:CURR? %s,%s" % (ch, v))
        dats = float(dats.split()[0])
        return dats
    ...
    def powerOvp(self, ch, v):
        self.send("DEVICE:POWER:OVP %s,%f" % (ch, v))

    def powerOcp(self, ch, v):
        self.send("DEVICE:POWER:OCP %s,%f" % (ch, v))

    # uvp
    def powerUvp(self, ch, v):
        self.send("DEVICE:POWER:UVP %s,%f" % (ch, v))

    def powerUcp(self, ch, v):
        self.send("DEVICE:POWER:UCP %s,%f" % (ch, v))
...
#########################################系统指令#######################################################
    ...
    def read_s6(self,d):
        data = self.query("PROJECT:S6:READ? %s"% (d))
        data = data.split()[0]
        return data

    def ADCLC_write(self,ch,k,b):
        self.send("POWER:FACTOR:ADCLC %s,%f,%f"% (ch,k,b))
    
    def ADCLC_read(self,ch):
        data =self.query("POWER:FACTOR:ADCLC? %s"% (ch))
        data = data.split(",")
        k = float(data[0])
        b = float(data[1].split()[0])
        return k,b

    def ADCHC_write(self,ch,k,b):
        self.send("POWER:FACTOR:ADCHC %s,%f,%f"% (ch,k,b))
    
    def ADCHC_read(self,ch):
        data = self.query("POWER:FACTOR:ADCHC? %s"% (ch))
        data = data.split(",")
        k = float(data[0])
        b = float(data[1].split()[0])
        return k,b
        
    def ADC_CUR_SWITCH(self,ch,x):
        self.send("POWE:CURRENT:SWITCH %s,%s"%(ch,x))
